...
type ParticipantRow = Omit<Participant, "joinedAt" | "leftAt"> & {
  ...
  visualAffectConsent: boolean; structuredSharingConsent: boolean;
...
function myParticipantDto(row: ParticipantRow): MyParticipant {
  return {
    ...
    consents: { transcription: row.transcriptionConsent, visualAffect: false, structuredSharing: row.structuredSharingConsent },
...
export async function createRoom(db: DatabaseExecutor, authUserId: string, title: string): Promise<CreateRoomData> {
  ...
  return { room: roomDto(room), lobbyPath: `/rooms/${room.id}/lobby` };
}
...
export async function getLobby(roomId: string, authUserId: string): Promise<LobbyData> {
  ...
  return {
    ...
    canJoin: row.status !== "ended" && row.mediaIsolated !== true && (row.myParticipantId !== null || row.participantCount < 12),
...
export async function joinRoom(
  ...
  if (!participant) {
    ...
    const inserted = await db<{ id: string }[]>`
      INSERT INTO participants (room_id, auth_user_id, display_name, role, transcription_consent,
        visual_affect_consent, structured_sharing_consent, consent_notice_version)
      VALUES (${roomId}::uuid, ${authUserId}::uuid, ${input.displayName},
        ...
        false, ${input.consents.structuredSharing}, ${CONSENT_NOTICE_VERSION}) RETURNING id`;
    ...
    await db`
      UPDATE participants SET display_name = ${input.displayName}, status = 'active', left_at = NULL,
        transcription_consent = ${input.consents.transcription}, visual_affect_consent = false,
  ...
  return { room: roomDto(room), me: myParticipantDto(participant), livekit, navigationPath: `/rooms/${roomId}` };
}
...
export async function updateConsents(db: DatabaseExecutor, roomId: string, authUserId: string, patch: ConsentPatch): Promise<ConsentData> {
  if (patch.visualAffect === true) throw new ApiProblem({ status: 409, code: "FEATURE_DISABLED", message: "Visual affect analysis is disabled." });
  ...
  const changed = transcription !== current.transcriptionConsent || sharing !== current.structuredSharingConsent;
  await db`UPDATE participants SET transcription_consent = ${transcription}, visual_affect_consent = false,
    structured_sharing_consent = ${sharing}, consent_revision = consent_revision + ${changed ? 1 : 0}
    WHERE id = ${current.id}::uuid`;
  if (current.structuredSharingConsent && !sharing) {
    const sessions = await db<{ id: string; nodeId: string }[]>`
      UPDATE mediation_sessions s SET status = 'cancelled', ended_at = clock_timestamp(), transition_error = 'CONSENT_WITHDRAWN'
      FROM mediation_members m WHERE m.mediation_session_id = s.id AND m.participant_id = ${current.id}::uuid
        AND s.status IN ('proposed', 'starting', 'active') RETURNING s.id, s.node_id AS "nodeId"`;
    if (sessions[0]) {
      await db`UPDATE participants SET media_isolated = false WHERE room_id = ${roomId}::uuid`;
      await db`UPDATE mind_map_nodes SET status = 'normal' WHERE id = ${sessions[0].nodeId}::uuid`;
      ...
      await emitRoomEvent(db, roomId, "room.mediation.changed", { mediationSessionId: sessions[0].id });
    } else if (changed) await emitRoomEvent(db, roomId);
  } else if (changed) await emitRoomEvent(db, roomId);
...
export async function leaveRoom(db: DatabaseExecutor, roomId: string, authUserId: string): Promise<ExitData> {
  ...
  await db`UPDATE participants SET status = 'left', left_at = COALESCE(left_at, clock_timestamp()),
    media_isolated = true, media_token_not_before = 'infinity'::timestamptz WHERE id = ${participant.id}::uuid`;
  await emitRoomEvent(db, roomId);
  const room = await requireRoom(db, roomId);
...
export async function endRoom(db: DatabaseExecutor, roomId: string, authUserId: string): Promise<ExitData> {
  ...
  await db`UPDATE rooms SET status = 'ended', active_mediation_session_id = NULL, active_mediation_node_id = NULL WHERE id = ${roomId}::uuid`;
  await db`UPDATE participants SET status = 'left', left_at = COALESCE(left_at, clock_timestamp()),
    media_isolated = true, media_token_not_before = 'infinity'::timestamptz WHERE room_id = ${roomId}::uuid`;
  await db`UPDATE mediation_sessions SET status = 'cancelled', ended_at = clock_timestamp(), transition_error = 'ROOM_ENDED'
    WHERE room_id = ${roomId}::uuid AND status IN ('proposed', 'starting', 'active')`;
  await emitRoomEvent(db, roomId);
